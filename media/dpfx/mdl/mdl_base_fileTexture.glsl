...
_base_textureReturn mdl_base_fileTexture( in sampler2D sampler, in float gamma, in vec3 colorOffset, in vec3 colorScale, in int monoSource, in _base_textureCoordinateInfo uvw
                                        , in vec2 cropU, in vec2 cropV, in int wrapU, in int wrapV, in bool clip )
{
  ...
  else
  {
    ...
    tr.tint = pow( t4.rgb, vec3( gamma ) ) * colorScale + colorOffset;
